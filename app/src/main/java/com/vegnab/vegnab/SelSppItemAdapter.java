        ...
        if (isPh == 1) { // a placeholder, assign a color for easier spotting
            if (lstN == 1) { // matched by Placeholder code
                if (isId == 0) { // not identified yet
                    v.setBackgroundColor(ContextCompat.getColor(ctx, R.color.match_placeholder_code));
                } else { // has been identified
                    v.setBackgroundColor(ContextCompat.getColor(ctx, R.color.match_placeholder_code_identified));
                }
            } else { // matched by Placeholder description
                if (isId == 0) { // not identified yet
                    v.setBackgroundColor(ContextCompat.getColor(ctx, R.color.match_placeholder_text));
                } else { // has been identified
                    v.setBackgroundColor(ContextCompat.getColor(ctx, R.color.match_placeholder_text_identified));
                }
            }
        } else { // not a placeholder, assign distinctive colors
            switch (lstN) {
                case 1: // a previously found code
                    v.setBackgroundColor(ContextCompat.getColor(ctx, R.color.match_previously_found_species_code));
                    break;
                case 2: // a previously found code, best match
                    v.setBackgroundColor(ContextCompat.getColor(ctx, R.color.match_previously_found_species_text));
                    break;
                case 3: // not previously found but a local species, match by code
                    v.setBackgroundColor(ContextCompat.getColor(ctx, R.color.match_local_species_code));
                    break;
                case 4: // not previously found but a local species, match by text
                    v.setBackgroundColor(ContextCompat.getColor(ctx, R.color.match_local_species_text));
                    break;
                case 5: // not a local species, match by code, low priority
                    v.setBackgroundColor(ContextCompat.getColor(ctx, R.color.match_nonlocal_species_code));
                    break;
                case 6: // not a local species, match by text, lowest priority
                    v.setBackgroundColor(ContextCompat.getColor(ctx, R.color.match_nonlocal_species_text));
                    break;
